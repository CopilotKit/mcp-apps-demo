...
export function createBoard(name, template = "blank") {
    const id = generateId("board");
    const board = { id, name, columns: TEMPLATES[template]() };
    boards.set(id, board);
    return board;
}
...
export function addCard(boardId, columnId, card) {
    ...
    const newCard = { ...card, id: generateId("card"), createdAt: now() };
    column.cards.push(newCard);
    return { success: true, message: `Added "${card.title}" to ${column.name}`, board, card: newCard };
}
export function updateCard(boardId, cardId, updates) {
    const board = boards.get(boardId);
    if (!board)
        return { success: false, message: "Board not found" };
...
export function moveCard(boardId, cardId, targetColumnId, position) {
    const board = boards.get(boardId);
    if (!board)
        return { success: false, message: "Board not found" };
