...
import { generateWorkout, adjustWorkoutDifficulty, completeExercise, skipExercise, getWorkoutProgress, getNextExercise, } from "./src/workout-logic.js";
import { getExerciseById } from "./src/exercises.js";
// Import recipe logic
import { generateRecipe, adjustServings as adjustRecipeServings, getRecipeById, } from "./src/recipes.js";
// Import trading logic
import { createPortfolio, executeTrade, refreshPrices, getStocks, } from "./src/stocks.js";
// Import kanban logic
import { createBoard, addCard, updateCard, deleteCard, moveCard, } from "./src/kanban.js";
...
const activePortfolios = new Map();
...
const getServer = async () => {
    ...
    const registerResource = (resource, htmlContent) => {
        server.registerResource(resource.name, resource.uri, resource, async () => ({
            contents: [
                {
                    uri: resource.uri,
    ...
    server.registerTool("workout-generator", {
        ...
        description: "Generates a personalized workout based on duration, focus area, equipment, and difficulty. Returns an interactive UI for the workout.",
        inputSchema: {
            duration: z
                .number()
    ...
    }, async ({ duration, focus, equipment, difficulty }) => {
    ...
    server.registerTool("log-exercise-complete", {
    ...
    }, async ({ workoutId, exerciseId, setsCompleted, feedback }) => {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `Completed ${exercise?.name || exerciseId} (${setsCompleted} sets). Progress: ${progress}%${feedback ? ` Feedback: ${feedback}` : ""}`,
                },
            ],
            structuredContent: {
                success: true,
    ...
    server.registerTool("adjust-workout", {
    ...
    }, async ({ workoutId, modification, exerciseId }) => {
        const workout = activeWorkouts.get(workoutId);
        if (!workout) {
            return {
                content: [{ type: "text", text: `Workout ${workoutId} not found.` }],
    ...
    server.registerTool("generate-recipe", {
        title: "Generate Recipe",
        description: "Generates a recipe based on cuisine, dietary preferences, and time constraints. Returns an interactive UI for the recipe.",
        inputSchema: {
            cuisine: z
    ...
    }, async ({ cuisine, dietary, servings, maxTime }) => {
        // Generate the recipe
    ...
    server.registerTool("adjust-servings", {
    ...
    server.registerTool("create-portfolio", {
        title: "Create Portfolio",
        description: "Creates an investment portfolio based on initial balance, risk tolerance, and focus area. Returns an interactive UI for trading.",
        inputSchema: {
            initialBalance: z
                .number()
                .min(1000)
    ...
    }, async ({ initialBalance, riskTolerance, focus }) => {
    ...
    server.registerTool("execute-trade", {
        title: "Execute Trade",
    ...
    }, async ({ portfolioId, symbol, action, quantity }) => {
    ...
    server.registerTool("refresh-prices", {
    ...
    server.registerTool("create-board", {
        ...
        description: "Creates a kanban board for project management with customizable columns and cards. Returns an interactive drag-drop UI.",
        inputSchema: {
    ...
    }, async ({ boardId, cardId, targetColumnId, position }) => {
    ...
    }, async ({ boardId, columnId, title, description, priority }) => {
    ...
    server.registerTool("delete-card", {
        title: "Delete Card",
