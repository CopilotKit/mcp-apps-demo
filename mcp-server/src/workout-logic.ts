...
// Focus area types that map to muscle groups
...
const EQUIPMENT_LEVELS: Record<EquipmentLevel, Equipment[]> = {
  none: ["none"],
...
function estimateExerciseTime(exercise: Exercise, sets: number, reps: number, restSeconds: number): number {
 ...
 */
export function generateWorkout(params: GenerateWorkoutParams): Workout {
  ...
  let eligibleExercises = allExercises.filter((ex) => {
    ...
    if (exerciseDiffIdx > targetDiffIdx) return false;
    ...
    if (!targetsMuscles && !isCardio) return false;
  ...
  for (const exercise of shuffled) {
    ...
    if (totalSeconds + exerciseTime > targetSeconds * 1.1) {
      // Allow 10% overflow
      if (workoutExercises.length >= 3) break; // Minimum 3 exercises
    }
    ...
    if (totalSeconds >= targetSeconds * 0.9) break;
  }
 ...
 */
export function getWorkoutProgress(workout: Workout): number {
 ...
 */
export function getNextExercise(workout: Workout): WorkoutExercise | null {
